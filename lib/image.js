...
var Image = Base.extend(
...

	{
		...
		convertToPerceptual: function () {
			...
			for(i = 0; i < len; i += 4) {
				this._convertRgbToXyz(image, i, pixelList);
				this._convertXyzToCieLab(image, i, pixelList);
			...
			for(i = 0; i < len; i += 4) {
				image[i] = 255 * ((pixelList[i] - bounds[0].min) / bounds[0].max);
				image[i + 1] = 255 * ((pixelList[i + 1] - bounds[1].min) / bounds[1].max);
				image[i + 2] = 255 * ((pixelList[i + 2] - bounds[2].min) / bounds[2].max);
			}
		...
		_convertRgbToXyz: function (buffer, offset, output) {
			...
			var result = [];
			result[0] = buffer[offset] * 0.4887180 + buffer[offset + 1] * 0.3106803 + buffer[offset + 2] * 0.2006017;
			result[1] = buffer[offset] * 0.1762044 + buffer[offset + 1] * 0.8129847 + buffer[offset + 2] * 0.0108109;
			result[2] = buffer[offset + 1] * 0.0102048 + buffer[offset + 2] * 0.9897952;
